...
export function Element(props) {
  ...
  if (props.icon.includes("conduit")) {
    elementDiv = (
      <div className={className}>
        <div className="conduit-number">{props.value}</div>
      </div>
    );
  } else if (props.icon.includes("base")) {
    const iconName =
      props.scale + " " + props.icon.split(" ")[0] + " location " + props.value;
    ...
    props.icon.includes("elevator") ||
    props.icon.includes("powerplant") ||
    props.icon.includes("techTile") ||
    props.icon.includes("fulfillAnyContract") ||
    props.icon.includes("techTile")
  ) {
  ...
  return <div className="scale-100">{elementDiv}</div>;
}
